    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
      ...
      <div className="relative bg-[#FBFBFD] w-full max-w-2xl h-auto max-h-[90vh] rounded-[32px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 ease-apple border border-white/50 ring-1 ring-black/5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200/50 bg-white/60 backdrop-blur-xl sticky top-0 z-10 shrink-0">
          <div className="flex items-center gap-3"><span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-widest ${currentCategoryColor.value} border border-black/5 shadow-sm`}>{category?.title}</span></div>
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar space-y-6">
          ...
          <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shadow-sm"><Calendar size={20} /></div>
                ...
                {isMultiDayEnabled && (
                    <div className="flex-1 border-l border-gray-200 pl-3">
                        ...
                        <input type="date" value={localTask.endDate || localTask.date} onChange={(e) => setLocalTask(prev => ({...prev, endDate: e.target.value}))} className="bg-transparent border-none outline-none focus:ring-0 p-0 text-slate-700 font-bold text-lg cursor-pointer font-sans" />
                    </div>
                )}
          </div>
          ...
          <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm focus-within:ring-2 focus-within:ring-slate-200 focus-within:border-slate-400 transition-all p-4">
                 ...
                 <textarea ref={descriptionRef} value={localTask.description} onChange={(e) => setLocalTask(prev => ({...prev, description: e.target.value}))} onKeyDown={handleDescriptionKeyDown} className="w-full min-h-[400px] p-0 text-slate-700 bg-transparent border-none outline-none focus:ring-0 text-base leading-relaxed placeholder-gray-300 resize-none" placeholder="Nhập chi tiết công việc..." />
              </div>
          </div>
        </div>

        <div className="p-6 border-t border-gray-200/50 bg-white/80 backdrop-blur-md flex justify-end shrink-0">
           <button onClick={handleSaveAndClose} className="flex items-center gap-2 px-8 py-3.5 bg-slate-900 text-white rounded-2xl hover:bg-black font-bold shadow-lg shadow-slate-300 transition-all transform active:scale-[0.98]"><Save size={18}/> Lưu thay đổi</button>
        </div>
      </div>
    </div>
  );
